"""SroScraperSource — GT World Challenge Europe source scraped from gt-world-challenge-europe.com.

Delegates all HTTP, cache, HTML-table parsing and session-mapping logic to
SroTimetableSource. Only GT World Challenge Europe-specific configuration
(base URL, championship) lives here — the circuit table is shared with
GT World Challenge America/Asia and IGTC (confirmed same CMS/URL scheme,
see ``sro_series/timetable_base.py`` module docstring for the full
investigation).

Source: https://www.gt-world-challenge-europe.com (``/calendar`` for the
...
class SroScraperSource(SroTimetableSource, GtwcEuropeSource):
    """GT World Challenge Europe source backed by gt-world-challenge-europe.com's HTML.

    Inherits all HTTP, cache, HTML-parsing and session-mapping logic from
    SroTimetableSource. The two members below are the only
    GT World Challenge Europe-specific configuration.
    """
