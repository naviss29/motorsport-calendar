...
from motorsport_calendar.providers.moto3.provider import Moto3Provider
from motorsport_calendar.providers.moto3.source import Moto3Source


def _make_source(events: list[Event] | None = None) -> Moto3Source:
    source = MagicMock(spec=Moto3Source)
...
class TestProviderIdentity:
    def test_name_is_moto3(self) -> None:
        provider = Moto3Provider(_make_source())
        assert provider.name == "moto3"

    def test_supported_championships_contains_moto3(self) -> None:
        provider = Moto3Provider(_make_source())
        assert "moto3" in provider.supported_championships
...
class TestFetchChampionship:
    async def test_returns_championship_object(self) -> None:
        provider = Moto3Provider(_make_source())
        champ = await provider.fetch_championship("moto3", 2026)
        assert champ is not None

    async def test_championship_name(self) -> None:
        provider = Moto3Provider(_make_source())
        champ = await provider.fetch_championship("moto3", 2026)
        assert champ.name == "Moto3"
    ...
    async def test_championship_id_contains_moto3(self) -> None:
        provider = Moto3Provider(_make_source())
        champ = await provider.fetch_championship("moto3", 2026)
        assert "moto3" in champ.id
...
class TestFetchEvents:
    async def test_delegates_to_source_get_season(self) -> None:
        source = _make_source()
        provider = Moto3Provider(source)
        await provider.fetch_events("moto3", 2026)
    ...
    async def test_returns_events_from_source(self) -> None:
        ...
        provider = Moto3Provider(_make_source(events))
        result = await provider.fetch_events("moto3", 2026)
        assert len(result) == 2
    ...
    async def test_year_passed_to_source(self) -> None:
        source = _make_source()
        provider = Moto3Provider(source)
        await provider.fetch_events("moto3", 2027)
