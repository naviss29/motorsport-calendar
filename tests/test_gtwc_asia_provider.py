...
from motorsport_calendar.providers.gtwc_asia.provider import GtwcAsiaProvider
from motorsport_calendar.providers.gtwc_asia.source import GtwcAsiaSource


def _make_source(events: list[Event] | None = None) -> GtwcAsiaSource:
    source = MagicMock(spec=GtwcAsiaSource)
    source.get_season = AsyncMock(return_value=events or [])
    return source


def _make_event(name: str = "Sepang International Circuit", season: int = 2026) -> MagicMock:
...
class TestProviderIdentity:
    def test_name_is_gtwc_asia(self) -> None:
        provider = GtwcAsiaProvider(_make_source())
        assert provider.name == "gtwc-asia"

    def test_supported_championships_contains_gtwc_asia(self) -> None:
        provider = GtwcAsiaProvider(_make_source())
        assert "gtwc-asia" in provider.supported_championships
...
class TestFetchChampionship:
    async def test_returns_championship_object(self) -> None:
        provider = GtwcAsiaProvider(_make_source())
        champ = await provider.fetch_championship("gtwc-asia", 2026)
        assert champ is not None

    async def test_championship_name(self) -> None:
        provider = GtwcAsiaProvider(_make_source())
        champ = await provider.fetch_championship("gtwc-asia", 2026)
        assert champ.name == "GT World Challenge Asia Powered by AWS"
    ...
    async def test_championship_id_contains_gtwc_asia(self) -> None:
        provider = GtwcAsiaProvider(_make_source())
        champ = await provider.fetch_championship("gtwc-asia", 2026)
        assert "gtwc-asia" in champ.id
...
class TestFetchEvents:
    async def test_delegates_to_source_get_season(self) -> None:
        source = _make_source()
        provider = GtwcAsiaProvider(source)
        await provider.fetch_events("gtwc-asia", 2026)
        source.get_season.assert_awaited_once_with(2026)

    async def test_returns_events_from_source(self) -> None:
        events = [_make_event("Sepang"), _make_event("Fuji Speedway")]
        provider = GtwcAsiaProvider(_make_source(events))
        result = await provider.fetch_events("gtwc-asia", 2026)
        assert len(result) == 2
    ...
    async def test_year_passed_to_source(self) -> None:
        source = _make_source()
        provider = GtwcAsiaProvider(source)
        await provider.fetch_events("gtwc-asia", 2027)
