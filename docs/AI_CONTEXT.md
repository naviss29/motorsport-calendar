...
> Dernière mise à jour : 2026-07-06 (Sprint 22)
...
21. **GUI Desktop Phase 1 (Sprint 22)** — Package `motorsport_calendar/gui/` : `models.py` (GenerateState), `controller.py` (list_championships + async generate_calendar), `main_view.py` (Flet UI), `app.py` (ft.run), `__main__.py`. Dépendance optionnelle `flet>=0.80`. Entrée script `motocal-gui`. 32 tests GUI (12 models + 20 controller, sans Flet). Flet 0.85 : `ft.run()`, `ft.Button`, `ft.Icons`, `ft.Colors`, `FilePicker.save_file()` async.
